...
// bot/commands/trade_now.js
const { fetchLatestAvaxSignal, hashSignal } = require('../../packages/lib/signal_avax');
const { openFromSignal } = require('../../packages/lib/hlTrade');
const { users } = require('../../packages/lib/db');
const { storeStrategyEncrypted } = require('../../packages/lib/strategyStore');
...
module.exports = async function tradeNow(ctx) {
  try {
    // 1) AVAX signal
    const s = await fetchLatestAvaxSignal();
    if (!s.ok) return ctx.reply(`⚠️ Could not fetch AVAX signal: ${s.reason || 'unknown'}`);
    const slot = s.slot;
    const sigHash = s.sigHash || hashSignal(slot);

    if (slot.signal === 'HOLD') {
      return ctx.reply('⏸️ Latest AVAX signal is HOLD — no trade.');
    }
    ...
    const already =
      ...
      u.lastExecuted.hash === sigHash &&
      u.lastExecuted.signal === slot.signal &&
      u.lastExecuted.symbol === 'AVAX';

    if (already) {
      return ctx.reply('⚪ This AVAX signal already executed for you. Waiting for the next one.');
    }

    // 3) Place AVAX-PERP order
    const exec = await openFromSignal(ctx.from.id, slot, { coin: 'AVAX-PERP' });
    if (exec.skipped) return ctx.reply('⏸️ Signal was HOLD — no trade.');
    ...
    // 5) Optional: store encrypted strategy on-chain
    try {
      const u2 = await col.findOne({ telegramId: String(ctx.from.id) }, { projection: { agentId: 1 } });
      const agentId = u2?.agentId;
      if (agentId && process.env.STRATEGY_STORE_ADDRESS) {
        const strategy = {
          ...
          source: 'AVAX-feed',
          slot,
          executed: { market: 'AVAX-PERP', side: exec.side, size: exec.size, leverage: exec.leverage },
        };
        const tx = await storeStrategyEncrypted({
          telegramId: ctx.from.id,
          agentId,
